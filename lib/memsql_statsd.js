...
Backend.prototype._process_config = function(statsd_config) {
    ...
    if (!_.has(this.config, 'host')) { throw new Error('Missing host in MemSQL configuration.'); }
    else if (!_.isString(this.config.host)) { throw new Error('MemSQL host must be a string.'); }
...
Backend.prototype.flush = function(timestamp, metrics) {
    ...
    for (key in metrics.counters) {
        ...
        this.cache.record([key, 'counter', 'count'].join('.'), value, timestamp);

        if (_.has(metrics, 'counter_rates')) {
            var value_per_second = metrics.counter_rates[key];
            this.cache.record([key, 'counter', 'rate'].join('.'), value_per_second, timestamp);
        }
    }
    ...
    for (key in metrics.timer_data) {
        for (var timer_data_key in metrics.timer_data[key]) {
            ...
            if (typeof(value) === 'number') {
                this.cache.record([key, 'timer', timer_data_key].join('.'), value, timestamp);
            } else {
                for (var timer_data_sub_key in value) {
                    this.cache.record(
                        [key, 'timer', timer_data_key, timer_data_sub_key].join('.'),
    ...
    for (key in metrics.gauges) {
        this.cache.record([key, 'gauge'].join('.'), metrics.gauges[key], timestamp);
    }

    // process sets
    for (key in metrics.sets) {
        this.cache.record([key, 'set', 'count'].join('.'), metrics.sets[key].values().length, timestamp);
    }
