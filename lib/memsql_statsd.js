...
var Backend = function(startup_time, config, events) {
    ...
    this.pool = new AggregatorsPool(this.config.host, this.config.port, this.config.user, this.config.password);
    this.cache = new AnalyticsCache(this.config.database, this.pool);
    this.stats = {
        last_exception: null,
        last_stat: startup_time
    };
